...
import in.dminc.auth.repositories.RefreshTokenRepository;
...
public class RefreshTokenService {
        ...
        if (refreshToken == null) {
            refreshToken =
                    RefreshToken.builder()
                            ...
                            .expiresAt(Instant.now().plusMillis(5 * 60 * 60 * 10000))
                            .user(user)
                            .build();
    ...
    public RefreshToken verifyRefreshToken(String refreshToken) {
        ...
        if (refToken.getExpiresAt().compareTo(Instant.now()) < 0) {
